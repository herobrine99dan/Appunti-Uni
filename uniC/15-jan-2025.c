...
//Hint: enqueu, dequeu
void append(LinkedNode *headNode, char *string) //Enqueu
{
...
char *pop(LinkedNode **headNode) //Dequeu 
{
...
char *rotateString(char *string) // Rotates string by one
{
    int size = strlen(string);
    char temp = string[0];
    // arancia→ranciaa
...
lunghezza è dispari, la stringa viene ruotata di 1 e quindi reinserita in coda. Tutte le stringhe
...
Si stampi a schermo il contenuto della coda Q dopo questa operazione.*/
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        {
            append(*headNode, rotateString(string));
        }
        iterator = tempNext;
    }
}
