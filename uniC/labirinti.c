...
void printLabyrinth(int rows, int columns, char (*labyrinth)[columns])
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < columns; j++)
        {
            if (labyrinth[i][j] == 'S')
            {
                printf(ANSI_COLOR_GREEN "%c " ANSI_COLOR_RESET, labyrinth[i][j]);
            }
            else if (labyrinth[i][j] == 'X')
            {
                printf(ANSI_COLOR_CYAN "%c " ANSI_COLOR_RESET, labyrinth[i][j]);
            }
            else
            {
...
void generateMaze(int rows, int columns, char (*labyrinth)[columns], int x, int y)
{
    labyrinth[x][y] = '.';
    int directions[4][2] = {
        {1, 0},
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (newX > 0 && newX < rows - 1 && newY > 0 && newY < columns - 1)
        {
            ...
            {
                labyrinth[newX - directions[i][0]][newY - directions[i][1]] = '.';
...
void putExit(int rows, int columns, char (*labyrinth)[columns])
{
    while (1)
    {
        int y = rand() % columns;
        if (labyrinth[y][rows - 2] == '.')
        {
            labyrinth[y][rows - 1] = '.';
            return;
        }
...
void initializeLabyrinth(int rows, int columns, char (*labyrinth)[columns])
{
...
int mazeTraverse(int rows, int columns, char (*labyrinth)[columns], int x, int y)
{
    // Safety checks
    if (x < 0 || y < 0)
    {
...
int main()
{
    ...
    char labyrinth[12][12] = {
        ...
        {'#', '.', '.', '.', '.', '.', '.', '#', '.', '.', '.', '#'},
        {'#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#'}};
    /*  printLabyrinth(12, 12, labyrinth);
 mazeTraverse(12, 12, labyrinth, 2, 0);
 printLabyrinth(12, 12, labyrinth); */

    initializeLabyrinth(12, 12, labyrinth);
    generateMaze(12, 12, labyrinth, 2, 0);
    putExit(12, 12, labyrinth);
    mazeTraverse(12, 12, labyrinth, 2, 0);
    printLabyrinth(12, 12, labyrinth);
}
